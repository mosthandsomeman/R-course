...
```{r echo=TRUE, message=FALSE, warning=FALSE}
gmp <- gmp %>% mutate(pop = gmp/pcgmp, nlmfit.125 = 6611*pop^(1/8), nlmfit.1 = 6611*pop^(0.1), nlmfit.15 = 6611*pop^(0.15))
...
gmp %>% ggplot() + geom_point(aes(x = pop, y = pcgmp))+
  ...
  scale_x_log10()+
  geom_line(aes(x = pop, y = nlmfit.125), col = 'blue', size = 1.5)+
    geom_line(aes(x = pop, y = nlmfit.1), col = 'red', size = 1.5)+
    geom_line(aes(x = pop, y = nlmfit.15), col = 'black', size = 1.5)
```
...
```
```{r echo=TRUE, message=FALSE, warning=FALSE}
mse <- function(para, response=gmp$pcgmp, predictor = gmp$pop){
  meanSqE <- sum((response - para[1]*predictor^para[2]) ^ 2) / nrow(gmp)
  return (meanSqE)
}
mse(c(6611,0.15))
mse(c(5000,0.10))
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
nlm(mse, c(y0=6611,a=1/8))
nlm(mse, c(y0=5555,a=0.1))
nlm(mse, c(y0=7777,a=0.15))
```
What do the quantities `minimum` and `estimate` represent? What values does it return for these?

minimum:the value of the estimated minimum of mse().
estimate:the point at which the minimum value of mes() is obtained.
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
plm <- function(y0, a, response=gmp$pcgmp, predictor = gmp$pop){
  nlmRes <- nlm(mse, c(y0, a), response, predictor)
  res <- list(y0 = nlmRes$estimate[1], a = nlmRes$estimate[2],mseVal = nlmRes$minimum)
  return(res)
}
...
plm(5000, 0.10)
```
When y0 = 6611, a = 0.15, MSE is small.
...
sde.pcgmp <- sd(gmp$pcgmp) / sqrt(nrow(gmp))
...
b.
```{r echo=TRUE, message=FALSE, warning=FALSE}
meanOmit <- function(response=gmp$pcgmp, omit){
  return(mean(response[-omit]))
}
```

c.
```{r echo=TRUE, message=FALSE, warning=FALSE}
jackknifed.means <- vector(length = nrow(gmp))
for(omit in 1:nrow(gmp)){
  jackknifed.means[omit] <- meanOmit(gmp$pcgmp, omit)
}
```

d.
```{r echo=TRUE, message=FALSE, warning=FALSE}
mean.jack <- mean(jackknifed.means)

var.jack <- (nrow(gmp) - 1) ^ 2 * var(jackknifed.means) / (nrow(gmp))

sde.jack <- sqrt(var.jack)
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
plm.jackknife <- function(y0, a, response=gmp$pcgmp, predictor = gmp$pop){
    jackknifed.means.y0 <- vector(length = nrow(gmp))
    jackknifed.means.a <- vector(length = nrow(gmp))
    for(omit in 1:nrow(gmp)){
      plmRes <- plm(y0, a, gmp$pcgmp[-omit], gmp$pop[-omit])
      jackknifed.means.y0[omit] <- plmRes$y0
      jackknifed.means.a[omit] <- plmRes$a
    }
    #mean.jack.y0 <- mean(jackknifed.means.y0)
    var.jack.y0 <- (nrow(gmp) - 1) ^ 2 * var(jackknifed.means.y0) / (nrow(gmp))
    sde.jack.y0 <- sqrt(var.jack.y0)
    
    #mean.jack.a <- mean(jackknifed.means.a)
    var.jack.a <- (nrow(gmp) - 1) ^ 2 * var(jackknifed.means.a) / (nrow(gmp))
    sde.jack.a <- sqrt(var.jack.a)
    
    res <- list(sde.y0 = sde.jack.y0, sde.a = sde.jack.a)
    return(res)
}

plm.jackknife(6611, 0.125, gmp$pcgmp, gmp$pop)
```
...
gmp2013 <- gmp2013 %>% mutate(pop = gmp/pcgmp)

plm.jackknife(6611, 0.125, gmp2013$pcgmp, gmp2013$pop)
```
The model parameters change little.
