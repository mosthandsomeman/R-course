...
```{r}
with(ckm_nodes, table(adoption_date))
sum(is.infinite(ckm_nodes$adoption_date))
...
length(anum)
ckm_nodes <- ckm_nodes[anum, ] # or with dplyr
...
ckm_nodes %>% group_by(adoption_date) %>%
  summarize(counts = n()) %>% 
  ggplot(aes(x = adoption_date, y = counts))+
  geom_point()

ckm_nodes %>% group_by(adoption_date) %>%
  summarize(counts = n()) %>% arrange(adoption_date) %>% 
  mutate(cumcounts = cumsum(counts)) %>% 
  ggplot(aes(x = adoption_date, y = cumcounts))+
  geom_point()
```
...
```{r}
adopted.early.ind <- ckm_nodes$adoption_date <= 2
sum(adopted.early.ind)
adopted.early.index <- which(ckm_nodes$adoption_date <= 2)
```
...
```{r}
adopted.late.ind <- ckm_nodes$adoption_date > 14
sum(adopted.late.ind)
adopted.late.index <- which(ckm_nodes$adoption_date > 14)
```
...
```{r}
contacts.37 <- ckm_network[row.names(ckm_network) == '37',] == 1 & ckm_nodes$adoption_date <= 5
sum(contacts.37)
```

    b. What proportion of doctor 37’s friends do those two doctors represent? 
```{r}
sum(contacts.37) / sum(ckm_network[row.names(ckm_network) == '37',])
```
